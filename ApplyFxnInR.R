...
N <- 10
x1 <- runif(N)
x2 <- runif(N) #runif generates random derivatives of uniform distribution
d <- as.data.frame(cbind(x1,x2))
head(d)
# Loop: create a vector to store the results in
rowMeanFor <- vector("double", N) #double precision vector
?vector
for (i in seq_len(N)) {
...
# Apply() fxn:
# Example:Sum a matrix over all the columns
